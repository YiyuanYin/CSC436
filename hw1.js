...
const Product = class Product {
    ...
    static halfOff = (products) => {
        return products.map((p) => ({...p, price: p.price*0.5}))
    }
    ...
    static printProducts = (products) => {
        for (const {name, price, availability} of products) {
            console.log(`Product: ${name}, Cost: $${price.toFixed(2)}, Availability: ${availability === 'In Stock' ? 'Yes' : 'No'}`)
        }
    }
